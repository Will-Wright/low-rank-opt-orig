function [x0, x, r, signal,kernel,measurements,signalEst,kernelEst,measurementsEst] = deblur
...
close all;
%% Path 
%addpath(fullfile('../Dependencies','minFunc'));
%addpath(fullfile('../Dependencies','minFunc_2012'));
%addpath(fullfile('../Dependencies','minFunc','compiled'));
%addpath(fullfile('../Dependencies','minFunc','mex'));
...
normw = norm(blur_kernel,'fro');
blur_kernel = blur_kernel/normw;
w = zeros(L1,L2);
...
alpha_conv = vec(rwt.mdwt(mat(conv_wx_image),repmat(realsqrt(.5),1,2))).'; % ijamj mod
figure;
plot(alpha_conv); title('wavelet coefficients of the convolved image');

% [alpha_x,l] = wavedec2(x,4,'db1');
alpha_x = vec(rwt.mdwt(mat(x),repmat(realsqrt(.5),1,2))).'; % ijamj mod
figure;
...
[~,pac] = sort(abs(alpha_conv(:)),'descend'); Ind_alpha_conv = abs(alpha_conv)>=abs(alpha_conv(pac(9000))); % ijamj mod
...
[~,pax] = sort(abs(alpha_x(:)),'descend'); Ind_alpha_x = abs(alpha_x)>=abs(alpha_x(pax(5000))); % ijamj mod
...
% Ind_alpha_x = false(1,length(alpha)); % Do this if you want to kill support info. from original image % ijamj mod
Ind_alpha_conv = false(1,length(alpha)); % Do this if you want to kill support info. from blurred image % ijamj mod
...
m = m'; % MPF Jul 3, 2015: commented out.
...
c = vec(rwt.mdwt(mat(conv_wx_image),repmat(realsqrt(.5),1,2))).'; % ijamj mod
CC = @(x) rwt.midwt(mat(C*x(:)),repmat(realsqrt(.5),1,2)); % ijamj mod
CCT = @(x) (C'*vec(rwt.mdwt(mat(x),repmat(realsqrt(.5),1,2)))); % ijamj mod

%% Approximated convolved image

% x_hat = waverec2(C*m,l,'db1');
x_hat = rwt.midwt(mat(C*m(:)),repmat(realsqrt(.5),1,2)); % ijamj mod
...
x0 = [m;h];
x  = [mEst;hEst];
r  = (conv_wx-ifft2(fft2(xEst).*fft2(wEst)))/256;
signalEst = xEst*normx;
kernelEst = wEst*normw;
kernel = mat(BB(h))*normw;
% signalEst = xEst;
% kernelEst = wEst;
% kernel = mat(BB(h));
measurementsEst = ifft2(fft2(signalEst).*fft2(kernelEst));
measurements = conv_wx*normx*normw;
% measurements = conv_wx;
