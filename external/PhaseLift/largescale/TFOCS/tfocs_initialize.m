...
odef = struct( ...
    ...
    'mu',         0     ,...
    'fid',        1     ,...
    'stopCrit',   2     ,... % the TFOCS paper used stopCrit = 2
    'restart',    Inf   );
...
assert( ismember(class(projectorF),F_types),'projectorF is of wrong type' );
if nargin >= 4
...
C_x = 0;
%if isinf( C_x ),
%    C_x = proj_tsvd( ApplyZhandle, dimx, RNK );
%    if isinf( C_x ),
%        zero_x0 = false;
%        [ C_x, x ] = proj_tsvd(ApplyZhandle, dimx, RNK );
%    end
%end
if isempty( A_x ),
