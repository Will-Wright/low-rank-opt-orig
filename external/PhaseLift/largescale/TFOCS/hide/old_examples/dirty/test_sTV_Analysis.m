...
x_noisy = awgn( x_original, snr, 'measured');
...
%% How to do the above without relying on solver_sBPDN_WW
normTV           = linop_TV( [n1,n2], [], 'norm' );
opts.normW12     = normTV^2;
opts.normW22     = normWavelet^2;
opts.normA2      = 1; % "A" is the identity
...
prox        = { prox_l2( EPS ), proj_linf(proxScale1*alpha),...
                proj_linf(proxScale2*beta) };
W1          = linop_compose( W_tv, 1 / proxScale1 );
W2          = linop_compose( W_wavelet, 1 / proxScale2 );
solver = @(mu,x0,z0,opts) tfocs_SCD( [], { A, -b; W1,0; W2, 0 }, prox, mu, vec(x0), z0, opts );
[x,out,optsOut]= continuation(solver,5*mu, vec(x0), z0, opts, contOpts );
%% Now, same as above but with x >= 0 constraint
prox        = { prox_l2( EPS ), proj_linf(proxScale1*alpha),...
                proj_linf(proxScale2*beta), proj_Rplus };
solver = @(mu,x0,z0,opts) tfocs_SCD( [], { A, -b; W1, 0; W2, 0; 1,0 }, prox, mu, vec(x0), z0, opts );
[x,out,optsOut]= continuation( solver, 5*mu, vec(x0), z0, opts, contOpts );
X_wavelets_tv = mat(x);
%%
figure(1); clf;
