function [x,r,g,info] = spgl1_modified( A, b, tau, sigma, x, options )
%SPGL1  Solve basis pursuit, basis pursuit denoise, and LASSO\
% This version modified by Stephen Becker, June 15 2010 as part of TFOCS
% TFOCS version 1.0, code by Michael Grant (mcg@cvxr.com) and Stephen Becker (srbecker@caltech.edu)
% It accepts a error function
%
...
% SRB adding:
if isfield(options,'stopFunction')
...
EXIT_USER_DEFINED  = 10; % added by SRB
...
while 1
    ...
    % SRB adding:
    if stopFunction
        stat = EXIT_USER_DEFINED * stopFunction( x, f );
    end
...
switch (stat)
   ...
   % SRB adding:
   case EXIT_USER_DEFINED
      printf('\, EXIT -- User defined termination criteria is satisfied\n');
   otherwise
