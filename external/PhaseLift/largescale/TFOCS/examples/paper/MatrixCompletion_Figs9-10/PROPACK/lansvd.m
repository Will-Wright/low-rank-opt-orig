...
% "neig" is the number of "good" eigenvalues
while neig < k
    ...
    i=1;
    neig = 0;
    while i<=min(j,k)  % SRB: should vectorize this for transparency
...
%     mn = min( abs( S(1: min(j,k))) );
    mn = min( abs( S(1:neig) ) ); % update 4-28-2010
    if mn > minSingValue
        k = k + increaseK; 
        ...
        % Bug fix, 4-28-2010.  If j > k, need to update k
        k = max(k,j);
