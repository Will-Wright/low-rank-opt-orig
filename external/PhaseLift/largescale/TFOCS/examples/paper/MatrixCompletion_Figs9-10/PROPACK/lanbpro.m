...
complex_tol = 1000000*eps;  % SRB adding - 5/13/09
...
%     end
    ...
    if isfield(options,'waitbar'),  waitb   = 1;                end
end
...
if isempty(U)
    ...
    int = [1:j]';
    % SRB: adding in support for complex numbers.
    % There are two versions of reorth -- a mex file (faster)
    % and a .m file (still rather fast).  The .m file can handle
    % complex numbers.  If the mex file exists, but we have complex
    % numbers, then force Matlab to call the .m file
    % To do this, I have copied reorth.m to reorth_complex.m
    % (i.e. files are identical), so that the .mex file won't
    % shadow it.
    if isreal(U) && isreal(p)
        [p,beta(j+1),rr] = reorth(U,p,beta(j+1),int,gamma,cgs);
    else
        [p,beta(j+1),rr] = reorth_complex(U,p,beta(j+1),int,gamma,cgs);
    end
...
for j=j0:k
    ...
    if j==1
        ...
        if alpha(j)<gamma*beta(j) && elr && ~fro
            ...
            while ~stop
                ...
                % SRB: adding in support for complex numbers
                if ~isreal(t)
                    if norm(imag(t)) > complex_tol*sqrt(length(r))
                        fprintf('imag(t) is %e\n',imag(t));
                        fprintf('Try setting OPTIONS.eta to 1e-16\n');
                        error('lanbpro: failure to stay real');
                    else
                        t = real(t);
                    end
                end
        ...
        if onesided~=-1 && ( fro || numax(j) > delta || force_reorth ) && alpha(j)~=0
            ...
            % Reorthogonalize v_j
            % SRB: see comments above at the other reorth
            if isreal(V) && isreal(r)
                [r,alpha(j),rr] = reorth(V,r,alpha(j),int,gamma,cgs);
            else
                [r,alpha(j),rr] = reorth_complex(V,r,alpha(j),int,gamma,cgs);
            end
    ...
    if alpha(j) < max(n,m)*anorm*eps && j<k,
        ...
        for attempt=1:3
            ...
            int = [1:j-1]';
            % SRB: see comments above at reoth_complex
            if isreal(V) && isreal(r)
                [r,nrmnew,rr] = reorth(V,r,nrm,int,gamma,cgs);
            else
                [r,nrmnew,rr] = reorth_complex(V,r,nrm,int,gamma,cgs);
            end
    ...
    if onesided~=1 && (fro || mumax(j) > delta || force_reorth) && beta(j+1)~=0
        ...
        % SRB: see comments above at previous reorth_complex
        if isreal(U) && isreal(p)
            [p,beta(j+1),rr] = reorth(U,p,beta(j+1),int,gamma,cgs);
        else
            [p,beta(j+1),rr] = reorth_complex(U,p,beta(j+1),int,gamma,cgs);
    ...
    if beta(j+1) < max(m,n)*anorm*eps  && j<k,
        ...
        for attempt=1:3
            ...
            if isreal(U) && isreal(p)
                ...
                [p,nrmnew,rr] = reorth_complex(U,p,nrm,int,gamma,cgs);
            end
            npu = npu + rr*length(int(:));  nreorthu = nreorthu + 1;
            mu(int) = m2*eps;
            if nrmnew > 0
